...
> **Docker 部署说明**：如果使用 Docker 部署 AstrBot，需要先在容器内安装依赖和字体：
> ```bash
> docker exec -it astrbot /bin/bash
> pip install uv njust-schedule-mcp --break-system-packages
...
| `PORTAL_LOGIN_URL` | 登录地址 | `http://202.119.81.113:8080` |
...
2. 本服务器使用 requests + BeautifulSoup 抓取 NJUST 教务系统
3. 登录验证码通过 ddddocr 本地识别，自动重试
4. JSESSIONID 会话持久化到文件，过期自动重新登录
5. 课表/成绩/考试数据缓存到本地文件，减少重复请求
...
**A:** ddddocr 对部分验证码识别率不是 100%，默认会自动重试 3 次。如果频繁失败，可以增大重试次数：
...
**A:** 参考 [安装部署](#安装部署ubuntu-服务器) 中的 Docker 说明，需要在容器内安装字体和依赖。
